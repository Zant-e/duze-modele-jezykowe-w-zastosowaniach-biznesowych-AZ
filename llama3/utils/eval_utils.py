...
class HarnessModel(LM):
    ...
    def generate_until(self, requests) -> List[str]:
        ...
        Args:
            requests (list[Instance]): A list of Instance objects with property `args` which returns a tuple (context, until).
                context (str): Context string
                until ([str]): The string sequences to generate until. These string sequences
                    may each span across multiple tokens, or may be part of one token.

        Returns:
            list[str]: A list of strings continuationa
                continuation (str): The generated continuation.
        """
    ...
    def loglikelihood(self, requests) -> List[Tuple[float, bool]]:
        ...
        Args:
            requests (list[Instance]): A list of Instance objects with property `args` which returns a tuple (context, continuation).
                context (str): Context string, which may be empty.
                continuation (str): The continuation over which log likelihood will be calculated. If there is a word boundary, the space should be in the continuation. For example, context="hello" continuation=" world" is correct.

        Returns:
            list[tuple[float, bool]]: A list of pairs (logprob, isgreedy)
                logprob (float): The log probability of `continuation`.
                isgreedy (bool): Whether `continuation` would be generated by greedy sampling from `context`.
        """
        self.model.eval()
        results = []
        ...
        for request in requests:
            ...
            # eos=True doesn't work properly for lambada perplexity testing
    ...
    def loglikelihood_rolling(self, requests) -> List[Tuple[float]]:
        ...
        - Will use the full max context length of the model.
        ...
        Args:
            requests (list[Instance]): A list of Instance objects with property `args` which returns a tuple (context,).
                context (str): The string for which we are computing overall loglikelihood

        Returns:
            list[tuple[float]]: A list of tuples (logprob,)
